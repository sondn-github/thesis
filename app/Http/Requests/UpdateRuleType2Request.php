...
class UpdateRuleType2Request extends FormRequest
{
    /**
    ...
    public function rules()
    {
        ...
        foreach ($this->request->get('facts') as $key => $value) {
            $rules['facts.'.$key] = 'required|exists:facts,id';
        }
    ...
    public function messages()
    {
        ...
        foreach($this->request->get('scoresFrom') as $key => $val)
        {
            ...
            $messages['scoresFrom.'.$key.'.lte'] = '"'.__('knowledge.scoreRangeFrom').' số '.($key + 1).'" phải nhỏ hơn hoặc bằng :other.';
        }
