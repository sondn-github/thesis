...
@section('content')
    ...
    <div class="content-wrapper">
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item"><a
                                    href="{{route('admin.rulesType2.index')}}">{{__('layouts/header.rule2Management')}}</a></li>
        ...
        <div class="content">
            <div class="container-fluid">
                <form action="{{route('admin.rulesType2.update', $knowledge->id)}}" class="form-horizontal" role="form"
                      method="post">
                    @csrf
                    @method('PUT')
                                                    ...
                                                    <select class="form-control" id="fact" name="facts[]" required>
                                                        ...
                                                        @foreach($factsTypeComment as $fact)
                                                            <option value="{{$fact->code}}"
                                                                    @if($premise[0] == $fact->code) selected @endif>{{$fact->code}} - {{$fact->description}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="col-md-3">
                                                    <input type="text" class="form-control" id="scoreFrom"
                                                           placeholder="{{__("knowledge.enterScore")}}"
                                                           name="scoresFrom[]" value="{{$premise[1]}}" required>
                                                </div>
                                                <div class="col-md-3">
                                                    <input type="text" class="form-control" id="score"
                                                           placeholder="{{__("knowledge.enterScore")}}"
                                                           name="scoresTo[]" value="{{$premise[2]}}" required>
                                                </div>
