...
@section('content')
    ...
    <div class="site-section">
        <div class="container">
            <form action="{{route('expert.knowledge.update', $knowledge->id)}}" class="form-horizontal" role="form" method="post">
                @csrf
                @method('PUT')
                                            ...
                                            <div class="col-md-4">
                                                <select class="form-control" id="criteria" name="criteria[]" required>
                                                    ...
                                                    @foreach($criteria as $c)
                                                        <option value="{{$c->code}}" @if($premise[0] == $c->code) selected @endif>{{$c->code}} - {{$c->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="col-md-4">
                                                <select class="form-control" id="operator" name="operators[]" required>
                                                    <option>{{__('knowledge.select')}}</option>
                                                    <option value="0" @if($premise[1] == 0) selected @endif>>=</option>
                                                    <option value="1" @if($premise[1] == 1) selected @endif><=</option>
                                                    <option value="2" @if($premise[1] == 2) selected @endif>></option>
                                                    <option value="3" @if($premise[1] == 3) selected @endif><</option>
                                                </select>
                                            </div>
                                            <div class="col-md-3">
                                                <input type="text" class="form-control" id="score"
                                                       placeholder="{{__("knowledge.enterScore")}}" name="scores[]" value="{{$premise[2]}}" required>
                                            </div>
                                ...
                                <div class="col-md-4">
                                    <select class="form-control" id="conclusion" name="conclusion" required>
                                        ...
                                        @foreach($facts as $fact)
                                            <option value="{{$fact->code}}" @if($knowledge->conclusion == $fact->code) selected @endif>{{$fact->code}} - {{$fact->description}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-4">
